...
class mixed_model(nn.Module):
    ...
    def forward(self, x):
        ...
        x = (x - torch.Tensor([x_mean, v_mean]).cuda())/torch.Tensor([x_std, v_std]).cuda()
        action_probs = self.pretrained_model(x)
        #print(action_probs.size())
        actions = torch.arange(0, 3).double().unsqueeze(-1).cuda()
        #print(actions.size())
...
class NeuralNetwork:
    def __init__(self, input_size = 4, output_size = 2, batch_size = 10, model_name = 'ppo', checkpoint = None):
        ...
        self.model = mixed_model(self.input_size, self.output_size).to(self.device)
