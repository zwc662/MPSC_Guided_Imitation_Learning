...
from models.mlp_discriminator import Critic 
...
parser = argparse.ArgumentParser(description='PyTorch WGAIL example')
...
parser.add_argument('--clamp-lower', type=float, default=-0.01, metavar = 'G',
                    help='lower bound of the weights of the discriminator')
parser.add_argument('--clamp-upper', type=float, default=0.01, metavar = 'G',
                    help='upper bound of the weights of the discriminator')
...
parser.add_argument('--num-expert-traj', type = int, default = 2048, metavar='N',
                    help='number of expert trajectories (default: 2048)')
...
discrim_net = Critic(state_dim + action_dim)
...
expert_traj = expert_traj[0: args.num_expert_traj, :]
print("Sample %d expert trajectories" % expert_traj.shape[0])
...
def update_params(batch, i_iter):
    ...
    for _ in range(1):
        ...
        actions_ = torch.unsqueeze(actions, 1)
        g_o = discrim_net(torch.cat([states, actions_], 1))
        e_o = discrim_net(expert_state_actions)
        optimizer_discrim.zero_grad()
        discrim_loss = torch.mean(g_o) - torch.mean(e_o)
    ...
    for i_iter in range(args.max_iter_num):
        ...
        if args.save_model_interval > 0 and (i_iter+1) % args.save_model_interval == 0:
            ...
            for t in range(10000):
                ...
                # choose mean action
                action = policy_net.select_action(state_var.to(device))[0][0].detach().cpu().numpy()
                ...
                state = next_state
            print('Episode {}\t reward: {:.2f}'.format(1, reward_episode))
